...
class TiebaSpider:
    ...
    #     a = a.replace('\r', '')
    #     s = etree.HTML(a)
    #     title = s.xpath("//a[@class='j_th_tit']/@title")
    #     print(title)
    #     href = s.xpath("//a[@class='j_th_tit']/@href")
    #     print(href)
    #     content = []
    ...
    def get_content_list(self, html_str):
        html = etree.HTML(html_str)
        div_list = html.xpath("//code[@id='pagelet_html_frs-list/pagelet/thread_list']")[0]
        str_div_list = etree.tostring(div_list).decode()
        temp = re.findall('<!--([\s\S]*)-->', str_div_list)[0]
        temp = temp.replace('\n', '')
        temp = temp.replace('\r', '')
        str_list_element = etree.HTML(temp)
        content_list = []
        for i in str_list_element:
            item = {}
            item["title"] = i.xpath("//a[@class='j_th_tit']/@title")
            item["href"] = self.part_url + i.xpath("//a[@class='j_th_tit']/@href")
